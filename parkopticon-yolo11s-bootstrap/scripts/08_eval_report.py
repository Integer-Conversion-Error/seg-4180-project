...
def load_class_names(data_yaml_path):
    ...
    return {0: "vehicle", 1: "enforcement_vehicle", 2: "police_old", 3: "police_new"}
...
def compute_enforcement_present_metric(box_metrics, class_names):
    ...
    """
    enforcement_metric = {"accuracy": 0.0, "precision": 0.0, "recall": 0.0, "f1": 0.0}

    try:
        # Extract AP scores for enforcement classes (1, 2, 3)
        if not hasattr(box_metrics, "ap") or box_metrics.ap is None:
            logger.warning(
                "No per-class AP available for enforcement_present calculation"
            )
        ...
        if len(box_metrics.ap) > 0:
            regular_ap = (
                float(box_metrics.ap[0]) if box_metrics.ap[0] is not None else 0.0
            )
        ...
        for class_idx in [1, 2, 3]:
            if len(box_metrics.ap) > class_idx:
                ap_val = (
                    float(box_metrics.ap[class_idx])
                    if box_metrics.ap[class_idx] is not None
                    else 0.0
                )
...
def main():
    ...
    if hasattr(metrics, "box") and metrics.box is not None:
        ...
        # Overall metrics
        report["metrics"]["mAP50"] = (
            float(box_metrics.map50) if box_metrics.map50 is not None else 0.0
        )
        report["metrics"]["mAP50-95"] = (
            float(box_metrics.map) if box_metrics.map is not None else 0.0
        )
        report["metrics"]["precision"] = (
            float(box_metrics.mp) if box_metrics.mp is not None else 0.0
        )
        report["metrics"]["recall"] = (
            float(box_metrics.mr) if box_metrics.mr is not None else 0.0
        )
            ...
            for class_idx, class_name in class_names.items():
                try:
                    ...
                    if class_idx < len(box_metrics.ap50):
                        ap50_val = (
                            float(box_metrics.ap50[class_idx])
                            if box_metrics.ap50[class_idx] is not None
                            else 0.0
                        )
        ...
        enforcement_metric = compute_enforcement_present_metric(
            box_metrics, class_names
        )
...
"""
    ...
    if "mAP" in report["per_class_metrics"]:
        for class_name in class_names.values():
            ...
            map50_val = (
                report["per_class_metrics"].get("mAP50", {}).get(class_name, 0.0)
            )
