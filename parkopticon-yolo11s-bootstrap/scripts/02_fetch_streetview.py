...
def main():
    ...
    parser.add_argument(
        "--points", "-p", default="manifests/points.csv", help="Input points CSV"
    )
    parser.add_argument(
        "--manifest", "-m", default="manifests/images.csv", help="Output manifest"
    )
    parser.add_argument(
        "--out-dir", "-o", default="data/images_original", help="Output directory"
    )
    parser.add_argument(
        "--api-key", "-k", default=None, help="GSV API key (or env GSV_API_KEY)"
    )
    parser.add_argument(
        "--source",
        default="outdoor",
    ...
    for row in tqdm(points, desc="Fetching Street View"):
        ...
        metadata = get_street_view_metadata(
            location, heading, pitch, fov, radius, api_key, source=args.source
        )
