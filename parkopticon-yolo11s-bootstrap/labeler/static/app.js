...
async function loadImages() {
    const resp = await fetch(
        `/api/images?queue=${encodeURIComponent(queueFilter)}&include_excluded=${showExcluded ? 'true' : 'false'}`
    );
...
async function navigateTo(index) {
    ...
    if (index < currentIndex) {
        ...
        if (target && target.review_status === 'done') {
            await fetch(`/api/review/${target.image_id}?status=todo`, { method: 'POST' });
